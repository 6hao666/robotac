...
main() {
  local workspace package_file cpu_count default_build_jobs build_jobs
  local rosdep_sources
  local -a package_files packages current_packages sudo_cmd
  ...
  if [[ ${ROBOTAC_SKIP_SYSTEM_PACKAGES:-0} != 1 ]]; then
    ...
    ensure_rosdep_initialized "$rosdep_sources" "${sudo_cmd[@]}"
    rosdep update
