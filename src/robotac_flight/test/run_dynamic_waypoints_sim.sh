#!/usr/bin/env bash
# Hardware-isolated regression for runtime PoseArray waypoint injection.
set -euo pipefail

workspace_dir=$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)
master_port=${ROBOTAC_DYNAMIC_WAYPOINT_SIM_MASTER_PORT:-11325}
log_dir=$(mktemp -d "${TMPDIR:-/tmp}/robotac-dynamic-waypoint-sim.XXXXXX")
roscore_pid=
sim_pid=
controller_pid=
