#!/usr/bin/env bash
# Hardware-isolated regression for the MAVROS setpoint_raw subscriber gate.
set -euo pipefail

workspace_dir=$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)
master_port=${ROBOTAC_SETPOINT_GATE_SIM_MASTER_PORT:-11326}
log_dir=$(mktemp -d "${TMPDIR:-/tmp}/robotac-setpoint-gate-sim.XXXXXX")
roscore_pid=
sim_pid=
controller_pid=
...
[[ "${start_result}" == *"mavros_setpoint_raw_consumer_unavailable"* ]]
echo "Setpoint consumer gate regression passed."
