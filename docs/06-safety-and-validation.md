...
![PlantUML：从离线检查到受控实飞的检查顺序](./assets/plantuml/safety-validation.svg "PlantUML：受控实飞检查顺序")
...
- 飞机未解锁并处于落地状态；
- 本地位置未过期；
- 修正后的里程计已通过检查，并持续送入 MAVROS 外部视觉接口；
- estimator 姿态、水平相对位置与垂直位置有效；
- timesync 状态未过期且往返延迟在限制内；
- MAVROS 已订阅位置设定点。

这些检查通过，只表示程序收到的状态符合要求。它们不能证明坐标方向、PX4 融合参数、静止
漂移或实际控制响应正确。是否可以起飞，仍由现场负责人根据地面检查和飞机实际情况决定。
